...
public class Database  extends SQLiteOpenHelper {
    ...
    public boolean inputdata(TambahData list) {
    ...
    public void readdata(ArrayList<TambahData> daftar){
        Cursor cursor = this.getReadableDatabase().rawQuery("select todo, description, priority from "+nama_tabel, null);
        while (cursor.moveToNext()){
            daftar.add(new TambahData(cursor.getString(0), cursor.getString(1), cursor.getString(2)));
        }
    }
}
